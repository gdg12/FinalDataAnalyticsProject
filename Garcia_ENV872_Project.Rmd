...
abstract: Water bodies with higher concentrations of dissolved oxygen tend to have
  ...
  however, no trend was apparent. A temporal analysis of dissolved oxygen was performed;
...
### “Correlation of Oahu Water Quality Parameters"
### "Effect of Region on Range of Dissolved Oxygen Concentrations in Oahu"
### "The Effect of Sample Date on Dissolved Oxygen Concentrations in North Oahu"
### "The Effect of Sample Date on Dissolved Oxygen Concentrations in South Oahu"
### "The Effect of Sample Date on Dissolved Oxygen Concentrations in West Oahu"
### "The Effect of Sample Date on Dissolved Oxygen Concentrations in East Oahu"
### "The Effect of Sample Date on Dissolved Oxygen Concentrations in Oahu"
### "The Effect of Sample Date on Water Temperatures in Oahu"
### "The Effect of Sample Date on pH in Oahu"
### "The Effect of Sample Date on Salinity in Oahu"
### "Comparison of DO, Temperature, and pH in Oahu"
...
OahuDataCleanSummary<-OahuDataClean%>%
  group_by(Month)%>%
 summarize(MeanDO=mean(DO, na.rm=TRUE), MeanDOPercentSaturation=mean(DissolvedOxygenSaturation, na.rm=TRUE), MeanWaterTemperature=mean(Temperature, na.rm=TRUE) , MeanSalinity=mean(Salinity,na.rm=TRUE), MeanTurbidity=mean(Turbidity,na.rm=TRUE), MeanpH=mean(pH,na.rm=TRUE), MeanEnterococcus=mean(Enterococcus,na.rm=TRUE), meanCP=mean(CP.Result, na.rm=TRUE))
...
```{r, message=FALSE, warning=FALSE}
library(plyr)
OahuDataClean$Region<-revalue(OahuDataClean$LocationIdentifier, c('152'='South', '153'='South', '154'='South',  '155'='South', '156'='South', '157'='South', '158'='South', '159'='South', '160'='South','161'='South', '162'='South', '165'='South', '169'='North', '170'='North', '171'='North','172'='North','173'='North',   '175'='East', '176'='East', '177'='East', '179'='North', '181'='West', '182'='West', '183'='West', '184'='West', '185'='West',  '186'='West', '187'='West','188'='West', '189'='South', '191'='East', '193'='East',  '194'='East', '195'='East','196'='East','197'='East', '198'='East', '200'='South',  '201'='South','202'='South', '203'='South', '204'='South', '205'='East', '207'='East', '208'='East', '211'='South','212'='South', '213'='South', '214'='South', '215'='West', '216'='East','217'='South', '218'='North', '221'='East',  '222'='South',   '224'='West',  '225'='North', '226'='East',  '227'='East', '228'='South',  '229'='South', '230'='East', '231'='South',  '232'='North', '233'='West', '234'='South', '235'='South',  '236'='South', '237'='South', '238'='South', '239'='South', '240'='South', '241'='East', '242'='East', '243'='East', '244'='North', '245'='North',   '246'='North',  '247'='North',  '249'='West',  '250'='West', '251'='West', '252'='West',   '253'='East',  '254'='South', '255'='South',   '256'='South',  '257'='South', '258'='South', '259'='South', '260'='East',   '261'='East', '262'='East','263'='North',  '264'='North',  '265'='North',  '266'='North', '267'='North',  '268'='West', '269'='West', '270'='South', '271'='West', '272'='West',  '273'='West',  '274'='West', '275'='South', '276'='South','277'='South', '278'='South', '279'='South', '280'='South', '281'='East', '282'='East', '283'='West', '284'='East', '285'='East', '286'='East', '287'='East', '288'='North', '289'='North', '290'='North',  '291'='North', '292'='West', '293'='West', '294'='West', '295'='West', '296'='West', '297'='North','298'='North', '299'='North', '304'='East', '305'='South', '306'='North', '307'='South', '308'='South', '309'='West','310'='South', '311'='South', '312'='South', '313'='North', '314'='West', '315'='West','316'='South', '317'='South', '322'='South', '323'='South', '324'='East','325'= 'East', '326'='East', '327'='East', '328'='East', '329'='East',  '362'='East', '387'='East', '388'='North', '389'='East', '411'='South', '412'='South', '413'='South', '414'='South', '415'='South', '416'='South'))                                                    
```


##Remove Observations that are not in Oahu (errors in original dataset)
```{r}
OahuDataClean<-OahuDataClean[-c(18913,19032,19156, 19287, 19384, 19476, 19567, 19745, 19904, 20041, 109, 226, 341, 448, 782, 11859, 13295, 18976, 19228, 19352, 19516, 19685, 19846, 19971, 20104, 14806, 16777, 16805, 16826, 16887, 16921, 16950, 4957, 5061, 5120, 5163, 5206, 5259, 5300), ]
```


##Remove observations with 0 degrees Long and 0 degrees latitude (incorrect values)
```{r}
library(dplyr)
OahuDataClean<- filter(OahuDataClean, LocationIdentifier != 324 & 
                                     LocationIdentifier != 325 &  LocationIdentifier != 326 &
                                     LocationIdentifier != 327 & LocationIdentifier != 328 & LocationIdentifier != 329)
```
...
```{r}
 OahuDataClean2<-OahuDataClean %>%   ###pipe indicates then
  filter(Temperature!=0, Salinity!=0, DO!=0, DissolvedOxygenSaturation!=0, pH!=0, Turbidity!=0)   ###take only data with the observations =/0
```
...
TempbyDO <- 
  ...
  labs(title="The Effect of Water Temperature on DO Concentrations across Oahu", x="Water Temperature (degrees C)", y="Dissolved Oxygen (mg/L)")
print(TempbyDO) 
```
...
```{r, message=FALSE}
attach(OahuDataClean)
HawaiiModClean<-lm(DO~Enterococcus + Temperature +Salinity +pH + Turbidity + CP.Result, data=OahuDataClean)
...
```{r North Oahu, warning=FALSE, message=FALSE, fig.height =7, fig.width = 18, echo=FALSE}
NorthOahuTimeSeries<-ggplot(HawaiiWaterCleanOahuNorth, aes(x =Date, y =DO)) + 
  geom_point() +

  geom_vline(xintercept=as.Date("2009-01-12"),color="253494", origin= "1970-01-01", lty=2, lwd=2) +
  geom_vline(xintercept=as.Date("2017-10-19"), color="253494", lty=2, lwd=2)+
    geom_vline(xintercept=as.Date("2018-11-29"),color="253494", origin= "1970-01-01", lty=2, lwd=2)+

  geom_smooth(aes(x = Date, y = DO, span=0.1), se=FALSE, color="indianred3", linetype=1, size=2)+
...
###Change Date to be a date object
```{r}
HawaiiWaterCleanOahuSouth$Date<-as.Date(HawaiiWaterCleanOahuSouth$Date, format = "%m/%d/%y")
```
...
```{r South Oahu, warning=FALSE, message=FALSE, fig.height =7, fig.width = 18, echo=FALSE}
SouthOahuTimeSeries<-ggplot(HawaiiWaterCleanOahuSouth, aes(x =Date, y =DO)) + 
  geom_point() +
 
  geom_vline(xintercept=as.Date("2012-12-12"),color="253494", origin= "1970-01-01", lty=2, lwd=2) +
  geom_vline(xintercept=as.Date("2017-11-28"), color="253494", lty=2, lwd=2)+
  geom_vline(xintercept=as.Date("2018-12-04"),color="253494", origin= "1970-01-01", lty=2, lwd=2) +
  geom_smooth(aes(x = Date, y = DO, span=0.1), se=FALSE, color="indianred3", linetype=1, size=2)+
...
```{r West Oahu, warning=FALSE, message=FALSE, fig.height =7, fig.width = 18, echo=FALSE}
WestOahuTimeSeries<-ggplot(HawaiiWaterCleanOahuWest, aes(x =Date, y =DO)) + 
  geom_point() +
 
  geom_vline(xintercept=as.Date("2009-01-20"),color="253494", origin= "1970-01-01", lty=2, lwd=2) +
  geom_vline(xintercept=as.Date("2017-11-29"), color="253494", lty=2, lwd=2)+
  geom_vline(xintercept=as.Date("2018-11-14"),color="253494", origin= "1970-01-01", lty=2, lwd=2) +
  geom_smooth(aes(x = Date, y = DO, span=0.1), se=FALSE, color="indianred3", linetype=1, size=2)+
...
```{r East Oahu, warning=FALSE, message=FALSE, fig.height =7, fig.width = 18, echo=FALSE}
EastOahuTimeSeries<-ggplot(HawaiiWaterCleanOahuEast, aes(x =Date, y =DO)) + 
  geom_point() +
 
  geom_vline(xintercept=as.Date("2014-11-17"),color="253494", origin= "1970-01-01", lty=2, lwd=2) +
...
scale_color_manual(values=wes_palette(name="GrandBudapest2")) +
 ...
 ## geom_vline(xintercept=as.Date("2005-08-17"), color="253494", origin= "1970-01-01", lty=2)  ###Second Change Point for North Oahu 
  ...
  geom_smooth(aes(x = Date, y = Temperature, span=0.1, method="lm"), color="black", linetype=1, size=2) +
...
scale_color_manual(values=wes_palette(name="GrandBudapest2")) +
 ...
 ## geom_vline(xintercept=as.Date("2005-08-17"), color="253494", origin= "1970-01-01", lty=2)  ###Second Change Point for North Oahu 
  ...
  geom_smooth(aes(x = Date, y =pH, span=0.1, method="lm"), color="black", linetype=1, size=2) +
...
```{r Salinity, warning=FALSE, message=FALSE, fig.height =9, fig.width = 19}
library(wesanderson)
...
##Compute Mean DO, Mean Temp, and Mean Salinity by Oahu Sample Location
```{r}
